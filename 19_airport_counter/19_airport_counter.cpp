...
public:
	...
	AirportCounter(TIterator begin, TIterator end) {
		...
		while (begin != end) {
			++flights[static_cast<size_t>(*begin)];
			++begin;
		}
	}
	...
	void Insert(TAirport airport) {
		++flights[static_cast<size_t>(airport)];
	}
	...
	void EraseAll(TAirport airport) {
		flights[static_cast<size_t>(airport)] = 0;
	}
	...
	Items GetItems() const {
		//return sort(flights.begin(), flights.end());
		Items result;
		for (size_t i = 0; i < static_cast<size_t>(TAirport::Last_); ++i) {
			result[i] = { static_cast<TAirport>(i), flights[i] };
		}
		return result;
	}
